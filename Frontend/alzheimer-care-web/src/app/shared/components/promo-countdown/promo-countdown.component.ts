...
@Component({
  ...
  `,
  styleUrls: [] // I assume the css comes from the main styles.
})
export class PromoCountdownComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    console.log('[PromoCountdownComponent] Initialized with date:', this.dateFinPromo, 'size:', this.size);
  ...
  private tick(): void {
    ...
    if (diff <= 0) {
      if (!this.expired) console.log('[PromoCountdownComponent] Promo expired!');
      this.expired = true;
    ...
    if (this.urgent && !wasUrgent) {
      console.log('[PromoCountdownComponent] Promo is now urgent (<24h)');
    }
