...
import tn.esprit.sensorsimulator.Service.SeedService;
...
@RestController
@RequestMapping("/simulation/seed")
...
public class SeedController {
    ...
    public ResponseEntity<Map<String, Object>> executeSeed() {
        try {
            ...
            log.error("❌ Sensor seed execution failed", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(buildErrorResponse("SEED_FAILED", "Sensor seeding failed", e));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> clearSeed() {
        try {
            ...
            log.error("❌ Sensor seed clear failed", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(buildErrorResponse("CLEAR_FAILED", "Sensor seed clearing failed", e));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> getStatus() {
        try {
            ...
            log.error("❌ Sensor seed status check failed", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
